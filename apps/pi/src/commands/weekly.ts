...
import { loadPiAgentWeeklyData } from '../data-loader.ts';
import { log, logger } from '../logger.ts';

export const weeklyCommand = define({
	name: 'weekly',
	description: 'Show pi-agent usage by week',
	args: {
		json: {
			type: 'boolean',
	...
	async run(ctx) {
		...
		const piData = await loadPiAgentWeeklyData(options);
		...
		if (ctx.values.json) {
			...
			logger.box('Pi-Agent Usage Report - Weekly');
