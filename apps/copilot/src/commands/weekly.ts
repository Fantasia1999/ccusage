...
export const weeklyCommand = define({
	name: 'weekly',
	description: 'Show Copilot CLI token usage grouped by week',
	args: sharedArgs,
	async run(ctx) {
		...
		if (events.length === 0) {
			log(
				jsonOutput ? JSON.stringify({ weekly: [], totals: null }) : 'No Copilot usage data found.',
			);
			return;
		}
		...
		try {
			...
			if (jsonOutput) {
				log(JSON.stringify({ weekly: rows, totals }, null, 2));
				return;
			}

			logger.box(
				`Copilot Token Usage Report - Weekly (Timezone: ${ctx.values.timezone ?? DEFAULT_TIMEZONE})`,
			);
			...
			const table = new ResponsiveTable({
				...
				compactHead: ['Week', 'Models', 'Input', 'Output', 'Premium', 'Cost (USD)'],
