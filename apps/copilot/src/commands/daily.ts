...
export const dailyCommand = define({
	...
	async run(ctx) {
		...
		if (events.length === 0) {
			log(
				jsonOutput ? JSON.stringify({ daily: [], totals: null }) : 'No Copilot usage data found.',
			);
			return;
		}
		...
		try {
			...
			logger.box(
				`Copilot Token Usage Report - Daily (Timezone: ${ctx.values.timezone ?? DEFAULT_TIMEZONE})`,
			);
			...
			const table = new ResponsiveTable({
				...
				compactHead: ['Date', 'Models', 'Input', 'Output', 'Premium', 'Cost (USD)'],
