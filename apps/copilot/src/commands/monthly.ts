...
export const monthlyCommand = define({
	...
	async run(ctx) {
		...
		if (events.length === 0) {
			log(
				jsonOutput ? JSON.stringify({ monthly: [], totals: null }) : 'No Copilot usage data found.',
			);
			return;
		}
		...
		try {
			...
			logger.box(
				`Copilot Token Usage Report - Monthly (Timezone: ${ctx.values.timezone ?? DEFAULT_TIMEZONE})`,
			);
			...
			const table = new ResponsiveTable({
				...
				compactHead: ['Month', 'Models', 'Input', 'Output', 'Premium', 'Cost (USD)'],
